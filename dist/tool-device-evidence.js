...
import { runResultStatuses } from "./catalog.js";
...
import { assertInside, writeGeneratedFile } from "./tool-file-utils.js";
import { buildScriptReviewReport, chooseNextRunPhase } from "./tool-diagnostics.js";
...
import { buildImportChecklist, buildSampleImportGuide, buildSampleReadinessReport, buildSampleSceneAudit, focusedSampleGeneratedDir, focusedSampleRunbookPath, readinessAction, uniqueBlockers } from "./tool-sample-workflow.js";
import { buildAndroidDeviceRunbookMarkdown, buildArtifactIndexMarkdown, buildCodePlanMarkdown, buildConfigIntegrationAuditMarkdown, buildDeviceRunResultFormMarkdown, buildDeviceValidationChecklistMarkdown, buildFocusedHandoffPackMarkdown, buildFocusedPreflightMarkdown, buildFocusedScopeStatusMarkdown, buildIssueReportMarkdown, buildPortalEvidenceMarkdown, buildLocalConfigFormMarkdown, buildOfficialAccessMarkdown, buildOnboardingMarkdown, buildProgrammingContextMarkdown, buildProjectHandoffMarkdown, buildRemainingWorkMarkdown, buildRunReportMarkdown, buildRunSequenceMarkdown, buildSampleImportGuideMarkdown, buildSceneAuditMarkdown, buildSupportBundleMarkdown, buildWorkflowStateMarkdown } from "./tool-markdown.js";
import { buildCodePlan, buildConfigIntegrationAudit, buildAccountMaterialsReport, buildFocusedRunSequence, buildLocalConfigForm, buildLocalConfigValidationReport, buildLatestLogDiagnostic, buildOfficialAccessReport, buildOnboardingReport, buildPortalEvidenceReport, buildProgrammingContext, buildProjectHandoff, buildRemainingWorkReport, buildUnityEnvironmentReport, buildWorkflowState, buildImportChecklistAction, preflightCheck, preflightNextCall, readLocalConfigForRemoteValidation } from "./tool-services.js";
const focusedHandoffSampleIds = ["image-tracking", "cloud-recognition", "mega", "all"];
export async function buildFocusedRunReport(root, sample, maxScriptIssues) {
...
export async function buildFocusedPreflight(root, sample, platform, outputPath, maxScriptIssues) {
    const [accountMaterials, unityEnvironment, localConfig, importChecklist, sampleImportGuide, readiness, sceneAudit, scriptReview, workflowState, portalEvidence] = await Promise.all([
...
export async function buildArtifactIndex(root, sample) {
...
export function focusedHandoffPackSamples(sampleId) {
    return sampleId === "all"
        ? focusedSamples()
        : [findSample(sampleId)];
}
export async function buildFocusedHandoffPackPlan(input) {
...
export async function writeFocusedHandoffPack(input) {
...
export async function writeFocusedHandoffPackForSample(input, sample, outputPath, written, skipped) {
...
export function focusedHandoffPackArtifactSpecs(root, sample) {
...
export function focusedHandoffPackPurpose(name) {
    const purposes = {
...
export async function writePackFile(root, relativePath, markdown, overwrite, written, skipped) {
...
export function defaultFocusedOutputPath(sample, platform) {
    return platform === "android"
        ? `Builds/${sample.id}.apk`
        : `Builds/iOS/${sample.id}`;
}
export function focusedArtifactReadOrder(artifacts) {
    const priority = [
...
export function artifactPriority(relativePath, priority) {
    const normalized = relativePath.replace(/\\/g, "/");
    const index = priority.findIndex((name) => normalized.endsWith(name));
    return index === -1 ? priority.length : index;
}
export function focusedArtifactDefinitions(root, sample) {
    const base = path.relative(root, focusedSampleGeneratedDir(root, sample));
    return [
        {
            name: "Runbook",
...
export async function buildDeviceRunResultForm(root, sample, platform, device, buildOutputPath, notes) {
...
export function samplePassedEvidencePlaceholder(sample, recordedDevice) {
    if (sample.id === "mega") {
...
export function androidDeviceInteractionAction(sample) {
    if (sample.id === "cloud-recognition") {
...
export async function buildCompletionReport(root, sample, platform, outputPath, maxScriptIssues, maxLogBytes, maxLogIssues) {
    const [preflight, deviceValidation, latestLog, runResultArtifact] = await Promise.all([
...
export function chooseCompletionStatus(hasRunResult, runResultStatus, preflightPassed, deviceReady, realDeviceRunPassed, hasBlockingLogIssues) {
    if (!hasRunResult) {
        return "not-run";
    }
...
export async function readRunResultArtifact(root, sample) {
...
export async function readPortalEvidenceArtifact(root, sample) {
...
export function devicePortalEvidenceBlockers(root, sample, platform, portalEvidence) {
...
export function isPortalEvidenceReadyForSample(portalEvidence, sample) {
    return sample.id !== "cloud-recognition" || (portalEvidence.exists
...
export function portalEvidencePreflightDetail(portalEvidence, sample) {
    if (sample.id !== "cloud-recognition") {
...
export function parseRunResultStatus(markdown) {
    const value = parseMarkdownField(markdown, "Overall status");
    return runResultStatuses.includes(value)
        ? value
        : null;
}
export function parseYesNoField(markdown, field) {
...
export function parseNumberField(markdown, field) {
    const value = parseMarkdownField(markdown, field);
...
export function parseMarkdownField(markdown, field) {
    const escapedField = field.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const match = markdown.match(new RegExp(`^${escapedField}:\\s*(.+)$`, "m"));
    return match?.[1]?.trim() ?? null;
}
export function countMarkdownStepStatuses(markdown, status) {
    const escapedStatus = status.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return [...markdown.matchAll(new RegExp(`Status:\\s*${escapedStatus}\\b`, "g"))].length;
}
export function countPassedDeviceValidationSteps(markdown) {
    const stepBlocks = markdown.split(/\n(?=\d+\.\s+)/g);
    return stepBlocks.filter((block) => /Status:\s*passed\b/i.test(block) &&
        /(real[- ]?device|device validation|on[- ]?device|android device|ios device|iphone|ipad|pixel|camera\/tracking validation|tracking validation)/i.test(block)).length;
}
export function hasRecordedDevice(device) {
    return typeof device === "string" && isNonPlaceholderString(device) && !/^not recorded$/i.test(device.trim());
}
export function buildCompletionNextActions(completionStatus, preflight, deviceValidation, runResultArtifact, latestLog, root, sample, platform) {
...
export async function buildFocusedScopeStatus(root, platform, maxScriptIssues, maxLogBytes, maxLogIssues) {
...
export function buildFocusedReleaseEvidence(status) {
    return {
        schemaVersion: 1,
        generatedAt: new Date().toISOString(),
...
export async function readFocusedReleaseEvidence(evidencePath, platform) {
    ...
    const normalizedItems = items.map((item) => {
        ...
        return {
            ...
            completionStatus: item.completionStatus === "passed" ? "passed" : item.completionStatus === "failed" ? "failed" : item.completionStatus === "blocked" ? "blocked" : "not-run",
...
export function buildFocusedScopeNextActions(root, platform, items) {
...
export async function buildIssueReport(input) {
    ...
    const localConfig = await buildLocalConfigValidationReport(input.root, path.join(input.root, "ProjectSettings", "EasyAR", "easyar.local.json"));
    const artifactPaths = {
...
export async function buildDeviceValidationChecklist(root, sample, platform, device, buildOutputPath) {
    const readiness = await buildSampleReadinessReport(root, sample);
...
export function buildDeviceValidationSteps(sample, platform) {
...
export function sampleSpecificDeviceValidationSteps(sample) {
    if (sample.id === "cloud-recognition") {
        return [
            {
...
export function sampleDevicePassCriteria(sample) {
    if (sample.id === "cloud-recognition") {
        return [
...
export function sampleDeviceEvidencePrompts(sample) {
    if (sample.id === "cloud-recognition") {
        return [
...
export function defaultIssueReproductionSteps(sample, platform) {
    return [
...
export function sampleExpectedIssueBehavior(sample) {
...
//# sourceMappingURL=tool-device-evidence.js.map