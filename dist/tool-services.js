...
import { buildSampleReadinessReport, buildImportChecklist, buildSampleSceneAudit, uniqueBlockers, focusedSampleGeneratedDir } from "./tool-sample-workflow.js";
import { resolveProjectPath, exists, ensureDirectory, readUnityVersion, findFiles, buildUnityEnvironmentReport } from "./tool-project.js";
import { analyzeUnityLog, buildScriptReviewReport } from "./tool-diagnostics.js";
import { readCloudRecognitionConfig, findUnityLogCandidates, readLogTail, readJsonFile, buildLocalConfigValidationReport, envFirst, envPresenceItem, localConfigAction, isRecord, isNonPlaceholderString, sanitizeRunResultNotes, sanitizeIssueText, hasCompleteCloudRecognitionConfig } from "./tool-local-config.js";
import { buildArtifactIndex, buildCompletionReport, buildFocusedScopeStatus, buildDeviceValidationChecklist, readFocusedReleaseEvidence } from "./tool-device-evidence.js";
...
const focusedHandoffSampleIds = ["image-tracking", "cloud-recognition", "mega", "all"];
...
export function chooseSampleExpansionOrder(targetSamples) {
    const rank = new Map([
        ["hello-ar", 1],
        ["surface-tracking", 2]
    ]);
...
export function buildSampleExpansionPlan(sample, platform, unityVersion, includeOfficialApiTrack) {
    return {
        sampleId: sample.id,
        sampleName: sample.name,
...
export function sampleExpansionPassCriteria(sample) {
    if (sample.id === "mega") {
        return [
...
export function sampleExpansionRisks(sample) {
    if (sample.id === "mega") {
        return [
...
export async function buildDeploymentReadiness(projectPath, unityPath) {
...
export async function readFirstExistingFile(candidates) {
    let lastError = null;
...
export function check(id, ok, severity, detail) {
    return {
        id,
        ok,
        severity,
...
export async function fileContains(filePath, needle) {
    try {
...
export function deploymentNextActions(blockers, warnings, auth, requestedUnityPath) {
...
export async function buildProductionValidationReport(root, focusedEvidencePath, platform, unityPath, verificationEvidence, maxScriptIssues, maxLogBytes, maxLogIssues) {
    ...
    const missingOfficialEnv = officialContract.environment.required.filter((name) => {
        ...
        const configured = officialContract.environment.configured;
    ...
    const focusedEvidenceSource = root
        ...
        : focusedEvidencePath
            ...
            : "not-provided";
    const gates = [
        productionGate("release-manifest", "Release manifest and package install surface", releaseManifest.readyForInstallDocs, "docs/RELEASE_MANIFEST.md, package.json, bins, docs, logo, and required release files are present.", releaseManifest.readyForInstallDocs
            ? "Release manifest is complete enough for consumer install docs."
            : `Missing release file(s): ${releaseManifest.missingRequiredFiles.join(", ") || "unknown"}.`, "Run easyar_write_release_manifest after restoring missing release files."),
        productionGate("deployment-readiness", "Deployment readiness", deployment.ready, "Package metadata, official endpoint environment, Unity executable configuration, focused scope, and security checks have no blockers.", deployment.ready
            ? "easyar_deployment_readiness has no blocker checks."
            : `${deployment.blockers.length} deployment blocker(s) remain.`, deployment.nextActions[0] ?? "Run easyar_write_deployment_readiness and resolve blockers."),
        productionGate("official-api-contract", "Official EasyAR account API contract", officialContract.readyForProductionOfficialAccess, "Official account, license, downloads, and Cloud Recognition endpoint environment variables are configured.", officialContract.readyForProductionOfficialAccess
            ? "Official endpoint environment variables are configured."
            : `Missing official endpoint env: ${missingOfficialEnv.join(", ") || "none"}.`, "Connect production deployment to authorized EasyAR account APIs and set the required endpoint environment variables."),
        productionGate("verification-commands", "Local package and CI verification commands", verificationEvidence === "passed", "npm run release:check has passed for the commit being released, and EASYAR_RELEASE_REQUIRE_PRODUCTION_READY=1 npm run release:check passes before the final release tag or npm publish.", verificationEvidence === "passed"
            ? "Caller recorded verificationEvidence=passed."
            : "No current verification evidence was provided to this report.", `Run and record: ${releaseManifest.verificationCommands.join("; ")}. Then regenerate with verificationEvidence=passed.`),
        productionGate("unity-project-evidence", "Unity focused sample evidence", Boolean(root) || Boolean(focusedScopeStatus?.allFocusedSamplesComplete), `A Unity project path or safe focused release evidence file is provided so ${focusedSampleNames} run-through evidence can be verified.`, root
            ? `Project path: ${root}.`
            : focusedScopeStatus
                ? `Evidence file: ${focusedEvidencePath}; source=${focusedEvidenceSource}.`
                : "No projectPath or focusedEvidencePath was provided.", "Provide projectPath locally, or set EASYAR_RELEASE_EVIDENCE_PATH to a safe JSON evidence file exported with easyar_write_release_evidence."),
        ...focused.map((sample) => {
            const access = officialAccessReports.find((report) => report.sample.id === sample.id);
            return productionGate(`official-access/${sample.id}`, `${sample.name} official access`, Boolean(access?.readyForOfficialContent), "Registered-user account, license, downloads, and sample-specific official access checks pass against configured EasyAR endpoints.", access
                ...
                : "Official access was not checked.", access?.nextActions[0] ?? `Run easyar_write_official_access_report projectPath=/path/to/UnityProject sampleId=${sample.id} platform=${platform}.`);
        }),
        productionGate("focused-scope-run-through", `Focused ${focusedSampleNames} run-through`, Boolean(focusedScopeStatus?.allFocusedSamplesComplete), "All focused samples have COMPLETION_REPORT.md with runThroughComplete=true based on preflight, device validation, RUN_RESULT.md, and latest log evidence.", focusedScopeStatus
            ...
            : "Focused scope status was not generated because no projectPath was provided.", focusedScopeStatus?.nextActions[0] ?? `Run easyar_write_focused_scope_status projectPath=/path/to/UnityProject platform=${platform}.`)
    ];
...
export function productionGate(id, title, ok, requiredEvidence, currentEvidence, nextAction) {
    return {
        id,
        title,
        ok,
...
export async function buildAccountOnboardingReport(root, sample, platform, accountStage) {
    const auth = readAuthConfig();
...
export function buildFirstRunAccountGuide(stage, root, sample, platform, needsCloudRecognition, bundleIdentifier) {
    const projectPath = root ?? "/path/to/UnityProject";
    return {
        entryQuestion: "Do you already have an EasyAR account?",
        designPrinciple: "Registration and login are browser handoffs. MCP asks only for account stage and local project paths, then validates local evidence without collecting website credentials or EasyAR secret values.",
        stageModel: [
            {
        ...
        routes: [
            {
                ...
                guide: "Open the official EasyAR website registration entry in a browser, create the account, activate it if required, then sign in to the EasyAR development center.",
                mcpAfterUserReturns: [
                ...
                returnPrompt: "After the browser flow is done, tell the MCP client only the new account stage, for example accountStage=registered-not-logged-in or accountStage=logged-in."
            },
            {
                id: "registered-user",
                active: stage === "registered-not-logged-in",
                answer: "Yes, but I am not logged in now.",
                guide: "Log in through the official EasyAR development center in the browser. MCP should not receive the website password or SMS/email verification code.",
                mcpAfterUserReturns: [
                ...
                returnPrompt: "After the dashboard is visible, tell the MCP client accountStage=logged-in. Do not paste passwords or verification codes."
            },
            {
                id: "logged-in-user",
                active: stage === "logged-in" || stage === "has-license" || stage === "has-cloud-credentials",
                answer: "Yes, and I can access the development center.",
                guide: "Create or locate the EasyAR Sense license, then prepare local project credentials. Cloud Recognition also requires CRS AppId plus API KEY from the account service configuration.",
                mcpAfterUserReturns: [
                ...
                returnPrompt: "After local files are filled, ask MCP to validate presence with easyar_validate_local_config. Secret values stay in local files or environment variables."
            }
        ],
...
export function deriveAccountOnboardingStage(accountStage, auth, localConfig, cloudConfig, needsCloudRecognition) {
...
export function buildAccountHumanSteps(stage, needsCloudRecognition, bundleIdentifier, platform) {
    const steps = [
        {
            id: "register",
...
export function buildAccountMcpSteps(root, sample, platform, needsCloudRecognition) {
    const projectPath = root ?? "/path/to/UnityProject";
    return [
        {
...
export function buildAccountOnboardingBlockers(stage, auth, localConfig, cloudConfig, needsCloudRecognition, root) {
    const blockers = [];
...
export function buildAccountOnboardingNextActions(stage, blockers, root, sample, platform, needsCloudRecognition) {
    const actions = [];
    if (stage === "not-registered") {
...
export async function buildAccountMaterialsReport(root, sample, platform) {
    const auth = readAuthConfig();
    const localConfig = root ? await buildLocalConfigValidationReport(root) : null;
    const cloudConfig = root ? await readCloudRecognitionConfig(root) : {};
...
export function accountMaterialNextAction(item, localConfigPath) {
...
export async function readOfficialOpenApiContract() {
    return JSON.parse(await readFile(officialOpenApiPath, "utf8"));
}
export function buildOfficialApiContract(baseUrl, includeExamples) {
    const auth = readAuthConfig();
...
export async function buildAuthorizationStrategyReport(root, sample, platform, accountStage, preferredMode) {
    const auth = readAuthConfig();
    const localConfig = root ? await buildLocalConfigValidationReport(root) : null;
    const cloudConfig = root ? await readCloudRecognitionConfig(root) : {};
    const needsCloudRecognition = sample.id === "cloud-recognition";
...
export function chooseAuthorizationMode(preferredMode, officialApiReady, localKeyReady, portalReadyForKeyCollection) {
...
export function authorizationStrategyToolSequence(selectedMode, root, sample, platform) {
    const projectPath = root ?? "/path/to/UnityProject";
...
export function authorizationStrategyNextActions(selectedMode, root, sample, platform, needsCloudRecognition, officialApiReady, localKeyReady, portalReadyForKeyCollection) {
    const projectPath = root ?? "/path/to/UnityProject";
    const actions = [];
    if (!officialApiReady) {
...
export function buildOfficialApiHandoff(baseUrl, includeCurl, deploymentTarget) {
...
export function officialApiBackendTodo(endpointId) {
...
export function officialApiEndpointAcceptance(endpointId) {
    if (endpointId === "account-status") {
        return [
...
export function officialApiCanaryCommand(endpoint) {
...
export function buildOfficialApiContractExamples(baseUrl) {
    return [
        {
...
export async function buildOfficialAccessReport(root, sample, platform, packageKind) {
    const auth = readAuthConfig();
...
export function readRemoteValidationConfigFromEnv(sample) {
    return {
...
export function officialAccessCheck(id, ok, required, summary, detail) {
    return {
        id,
        ok,
        required,
...
export function officialAccessRemoteCheck(id, required, result) {
...
export async function buildPortalEvidenceReport(input) {
    ...
    const localCloudReady = !needsCloudRecognition || (isNonPlaceholderString(cloudConfig.appId)
    ...
    const localLicenseReady = localConfig
        ...
        : false;
    const blockers = [
        portalEvidenceBlocker("sense-license", input.senseLicenseStatus !== "missing", "Sense License record is missing in the observed portal page.", "Create or locate the EasyAR Sense license bound to the Unity bundle/package identifier."),
        portalEvidenceBlocker("local-license-config", !input.root || localLicenseReady, "Local easyar.local.json does not have a valid EasyAR license presence check.", "Fill easyar.licenseKey locally, then run easyar_validate_local_config."),
        portalEvidenceBlocker("cloud-service-enabled", !needsCloudRecognition || cloudServiceEnabled, "Cloud Recognition is not listed as enabled for the observed API KEY app.", "Enable Cloud Recognition for the API KEY app in the EasyAR development center."),
        portalEvidenceBlocker("cloud-api-key-presence", !needsCloudRecognition || input.apiKeyPresent === true, "Portal API KEY presence was not confirmed.", "Confirm the API KEY field exists in the portal, but do not paste the value into MCP chat."),
        portalEvidenceBlocker("cloud-api-secret-presence", !needsCloudRecognition || input.apiSecretPresent === true, "Portal API Secret presence was not confirmed.", "Confirm the API Secret field exists in the portal, but do not paste the value into MCP chat."),
        portalEvidenceBlocker("local-cloud-config", !input.root || localCloudReady, "Local Cloud Recognition credentials are not complete for the focused sample.", "Fill easyar.cloudRecognition.appId, serverAddress, apiKey, and apiSecret locally, then validate local config."),
        portalEvidenceBlocker("cloud-library", !needsCloudRecognition || hasCloudLibrary, "No Cloud Recognition image library was observed.", "Create a Cloud Recognition image library in the EasyAR development center."),
        portalEvidenceBlocker("cloud-target-image", !needsCloudRecognition || hasCloudTargets, "No uploaded Cloud Recognition target image count was observed.", "Upload at least one test target image to the Cloud Recognition library before device validation.")
    ...
    return {
        ...
        blockers,
        notes: input.notes.map((note) => sanitizePortalText(note)).filter((note) => Boolean(note)),
        nextActions: blockers.length > 0
...
export function portalEvidenceBlocker(id, ok, detail, action) {
    return {
        id,
        ok,
        detail,
...
export function sanitizePortalText(value) {
    if (!value) {
        return null;
    }
...
export function sanitizePortalUrl(value) {
    if (!value) {
        return null;
    }
    try {
...
export function looksLikeStandaloneSecret(value) {
    if (value === "<redacted>") {
        return false;
    }
    return /^(?:[A-Fa-f0-9]{24,}|[A-Za-z0-9+/=_-]{40,})$/.test(value);
}
export async function buildClientSetupReport(client, entrypointMode, serverPath, includeTokenPlaceholder) {
    const packageJson = await readPackageMetadata();
...
export function clientConfigDestination(client) {
...
export function buildClientAcceptanceChecklist(client, entrypointMode) {
    return [
...
export function buildClientTroubleshooting(client, entrypointMode) {
    return [
...
export function clientSetupCheck(id, ok, required, detail) {
    return {
        id,
        ok,
        required,
...
export function clientSetupAction(id) {
    if (id === "node-version") {
...
export function firstRunTopNextCall(root, sample, platform, accountStage, localConfigValid, missingMaterials) {
    const projectPath = root ?? "/path/to/UnityProject";
    if (!root) {
        return {
...
export async function buildRemainingWorkReport(input) {
    const focused = focusedSamples();
    const production = await buildProductionValidationReport(input.root, undefined, input.platform, undefined, input.verificationEvidence, input.maxScriptIssues, input.maxLogBytes, input.maxLogIssues);
    ...
    const categories = [
        ...
        remainingCategory({
            ...
            checks: [
                ...
                ...focused.map((sample, index) => remainingCheck(`official-access-${sample.id}`, Boolean(officialAccessGates[index]?.ok), officialAccessGates[index]?.currentEvidence ?? `${sample.name} official access was not checked.`, officialAccessGates[index]?.nextAction ?? `Run easyar_write_official_access_report for ${sample.id}.`))
            ]
        }),
        remainingCategory({
...
export function remainingCheck(id, ok, evidence, nextAction) {
    return {
        id,
        ok,
        evidence,
...
export function buildClientLaunch(entrypointMode, serverPath) {
...
export function buildClientConfig(client, launch, env) {
    if (client === "codex") {
        return {
            mcpServers: {
                easyar: {
...
export async function buildWorkflowState(root, sample, platform, outputPath, maxScriptIssues) {
...
export function chooseWorkflowNextState(input) {
    ...
    if (input.sample.implementationStatus !== "focused") {
        return workflowDecision("deferred-sample", true, "This sample is deferred. Current focused run-through scope is image-tracking and cloud-recognition.", {
    ...
    if (!input.importReady) {
        ...
        return workflowDecision("import-official-assets", true, `Missing required official import item(s): ${input.missingRequiredImportItems.join(", ")}.`, needsDownloadDiscovery
            ? {
    ...
    if (input.generatedHelpersMissing) {
        return workflowDecision("prepare-project", false, `Generated helper/runbook check(s) missing: ${input.failingReadinessChecks.join(", ")}.`, {
            tool: "easyar_prepare_unity_project",
            arguments: {
                ...baseArgs,
                overwrite: false
    ...
    if (!input.configValid) {
        return workflowDecision("configure-local-secrets", true, `Local config check(s) failing: ${input.failingConfigChecks.join(", ")}.`, {
            tool: "easyar_validate_local_config",
            arguments: {
                projectPath: input.root
            }
    ...
    if (!input.sceneReady) {
        return workflowDecision("configure-unity-scene", false, `Scene or Build Settings blocker(s): ${input.sceneBlockers.join(", ")}.`, {
            tool: "easyar_create_build_settings_helper",
            arguments: {
                ...baseArgs,
                platform: input.platform,
    ...
    if (input.scriptIssueCount > 0) {
        return workflowDecision("fix-scripts", true, `Static script review found ${input.scriptIssueCount} issue(s).`, {
    ...
    if (input.missingArtifacts.length > 0) {
        return workflowDecision("write-handoff-artifacts", false, `Missing handoff artifact(s): ${input.missingArtifacts.join(", ")}.`, {
    ...
    if (!input.deviceReady) {
        return workflowDecision("resolve-device-preflight", true, `Device validation blocker(s): ${input.deviceBlockers.join(", ")}.`, {
            tool: "easyar_write_device_validation_checklist",
            arguments: {
                ...baseArgs,
                platform: input.platform,
    ...
    return workflowDecision("build-and-run-device", false, "Focused sample preflight is ready for Unity compile, player build, and real-device validation.", {
        tool: "easyar_create_device_build_helper",
        arguments: {
            projectPath: input.root,
            platform: input.platform,
...
export function workflowDecision(phase, blocked, reason, nextCall, nextActions, recommendedSequence = []) {
    return {
        phase,
        blocked,
        reason,
...
export function buildImportChecklistAction(importChecklist) {
...
export function preflightCheck(id, ok, area, detail, action) {
    return {
        id,
        ok,
        area,
...
export function preflightNextCall(blocker, root, sample, platform) {
...
export async function buildCodePlan(root, sample, goal, targetFiles, maxScriptIssues) {
...
export async function buildCodeChangeSummary(root, sample, goal, targetFiles, notes, maxIssues) {
...
export async function buildProgrammingContext(root, sample, goal, maxFiles, maxIssues) {
...
export async function buildConfigIntegrationAudit(root, sample, maxFiles, maxCandidates) {
...
export async function scanConfigIntegrationFiles(root, relativePaths, kind, maxCandidates) {
    const candidates = [];
...
export function configIntegrationSignals(text) {
    const checks = [
...
export function redactedConfigSnippets(text) {
...
export function scoreConfigCandidate(candidate, sample) {
    const weights = {
...
export function buildCodeChangeNextActions(sample, issueCount, missingFiles) {
    const actions = [];
...
export function normalizeProjectRelativePath(root, relativePath) {
...
export function buildCodePlanImplementationSteps(sample, primaryFile, className, kind) {
    const common = [
...
export function buildCodePlanVerificationCalls(root, sample) {
    return [
        {
            tool: "easyar_review_csharp_scripts",
            arguments: {
...
export async function buildLatestLogDiagnostic(root, sample, maxLogBytes, maxLogIssues) {
...
export async function buildLocalConfigForm(root, sample, platform, accountStage, bundleIdentifierInput) {
    ...
    const needsCloudRecognition = sample.id === "cloud-recognition";
    const checkOk = (id) => localConfig.checks.some((check) => check.id === id && check.ok);
    const materialPresent = (id) => accountMaterials.materials.some((item) => item.id === id && item.present);
...
export async function buildLocalConfigHandoffReport(root, sample, platform, accountStage) {
...
export async function readLocalConfigForRemoteValidation(projectPath) {
...
export async function writeFocusedSampleSupportFiles(root, sample, overwrite, written) {
    if (sample.id === "image-tracking") {
        const targetDir = path.join(focusedSampleGeneratedDir(root, sample), "Targets");
        await mkdir(targetDir, { recursive: true });
        await writeGeneratedFile(path.join(targetDir, "README.md"), [
    ...
    if (sample.id === "cloud-recognition") {
        ...
        await writeGeneratedFile(path.join(cloudDir, "README.md"), [
    ...
    if (sample.id === "mega") {
        ...
        await writeGeneratedFile(path.join(megaDir, "README.md"), [
...
//# sourceMappingURL=tool-services.js.map