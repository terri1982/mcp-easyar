...
- Added Mega `BlockHolder.BlockRootSource` readiness validation and final localized block evidence for `大厅+办公室+阳台+GPS+0716`, preventing the `Block root not exist` runtime failure when the sample scene uses an empty external BlockRoot.
- Added safe Android real-device evidence for EasyAR Sense Unity Plugin `4003.0.0` Image Tracking, Cloud Recognition, Motion Tracking, and Mega Block sample startup.
- Hardened PackageCache sample import guidance and diagnostics around Unity `.meta` preservation so missing script and null texture GUID failures are surfaced before release validation.
...
- GitHub Release tarball install guide for local-key MVP users, including package-bin MCP config and first focused workflow calls.
- Local-key MVP release notes for GitHub prereleases, clarifying install commands, focused scope, security boundary, and deferred production API work.
