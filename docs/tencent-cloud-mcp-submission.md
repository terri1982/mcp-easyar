...
- Release: https://github.com/terri1982/mcp-easyar/releases/tag/v0.1.0-local-key.34
- Release tarball: https://github.com/terri1982/mcp-easyar/releases/download/v0.1.0-local-key.34/mcp-easyar-0.1.0.tgz
...
`mcp-easyar` 是面向 EasyAR Unity 授权开发流程的 MCP 服务。当前 local-key MVP 已覆盖 Image Tracking、CRS / Cloud Recognition，并正在扩展 Mega Sample。用户在自己的浏览器里完成 EasyAR 官网注册、登录、插件下载和 key 创建；MCP 负责引导 Unity 本地配置、检查工程状态、生成运行清单、验证构建准备情况，并生成已脱敏的排查材料。
...
- Ready for next local-key release: Mega local-key workflow with Android APK install/startup and localization-tracking evidence.
