...
Mega is the exception to the JSON route: configure its package License Key and Global Mega Block fields in `Assets/XR/Settings/EasyAR Settings.asset`, validate with `sampleId=mega`, and use the scene audit's recommended `scenePath` when Build Settings contains ambiguous Onsite/Simulator candidates.
...
For WeChat Mini Program samples, use `easyar-run-wechat-miniprogram` with `sampleId=wechat-mega` or `sampleId=wechat-crs`. The prompt keeps the flow on official EasyAR/WeChat browser and DevTools handoff, can create a minimal Mini Program workspace shell with `easyar_create_miniprogram_sample_workspace`, can find/write official package search evidence with `easyar_find_miniprogram_official_package` and `easyar_write_miniprogram_official_package_search`, writes local preflight/evidence artifacts, and requires real-device preview evidence before completion.

During Mini Program work, rerun `easyar_write_miniprogram_run_through_status` whenever the project, official package search/import, DevTools log, or real-device evidence changes. It writes one local status file with the next recommended MCP call.
