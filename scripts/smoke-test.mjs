...
try {
  ...
  const clientAcceptanceResource = await request("resources/read", {
    uri: "easyar://client/acceptance"
  });
  assertResourceIncludes(clientAcceptanceResource, "mcp-easyar Client Acceptance Checklist");
  ...
  const committedJapaneseReadme = await readFile(
    path.join(process.cwd(), "README.ja.md"),
    "utf8"
  );
  const committedVietnameseReadme = await readFile(
    path.join(process.cwd(), "README.vi.md"),
    "utf8"
  );
  const committedJapaneseDocsIndex = await readFile(
    path.join(process.cwd(), "docs", "ja", "README.md"),
    "utf8"
  );
  const committedVietnameseDocsIndex = await readFile(
    path.join(process.cwd(), "docs", "vi", "README.md"),
    "utf8"
  );
  ...
  const toolDeviceEvidenceSource = await readFile(path.join(process.cwd(), "src", "tool-device-evidence.ts"), "utf8");
  const mcpSource = `${serverSource}\n${toolsSource}\n${toolsRegistrySource}\n${toolServicesSource}\n${toolMarkdownSource}\n${toolFileUtilsSource}\n${toolLocalConfigSource}\n${toolDiagnosticsSource}\n${toolProjectSource}\n${toolSampleWorkflowSource}\n${toolDeviceEvidenceSource}`;
