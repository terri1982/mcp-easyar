...
export function buildFocusedPreflightMarkdown(preflight: Awaited<ReturnType<typeof buildFocusedPreflight>>): string {
  return [
    ...
    `Existing device build available: ${preflight.summaries.existingDeviceBuild ? "yes" : "no"}`,
    `Device blockers: ${preflight.summaries.deviceBlockers.length > 0 ? preflight.summaries.deviceBlockers.join(", ") : "none"}`,
    "",
    "## Blockers",
    "",
...
export function buildSceneAuditMarkdown(audit: Awaited<ReturnType<typeof buildSampleSceneAudit>>): string {
  return [
    ...
    ...(audit.sampleSpecific.cloudConfig
      ...
      : []),
    ...(audit.sampleSpecific.sceneSelection
      ? [
          "",
          "### Mega Scene Selection",
          "",
          `Onsite candidates: ${audit.sampleSpecific.sceneSelection.onsiteCandidates.join(", ") || "none"}`,
          `Simulator candidates: ${audit.sampleSpecific.sceneSelection.simulatorCandidates.join(", ") || "none"}`,
          `Selected first enabled scene: ${audit.sampleSpecific.sceneSelection.selectedFirstEnabledScene ?? "none"}`,
          `Selected scene valid for Onsite Mega: ${audit.sampleSpecific.sceneSelection.selectedSceneValid ? "yes" : "no"}`,
          `Recommended scene: ${audit.sampleSpecific.sceneSelection.recommendedScene ?? "none"}`,
          `Explicit scenePath required: ${audit.sampleSpecific.sceneSelection.requiresExplicitSelection ? "yes" : "no"}`,
          audit.sampleSpecific.sceneSelection.reason
        ]
      : []),
    "",
    "## Security",
...
export function buildAndroidDeviceRunbookMarkdown(runbook: Awaited<ReturnType<typeof buildAndroidDeviceRunbook>>): string {
  return [
    ...
    "## Status Meaning",
    "",
    "- `easyar_android_start_app` is accepted only when its `launchVerified` result is `true` and `foregroundPackage` equals the requested package.",
    "- `launchCommandSucceeded: true` by itself is insufficient: Android can return a successful `monkey` command while another app remains in the foreground.",
    "- For Mega, an app launch or a clean logcat is not a sample pass. The final result stays blocked until the selected Mega Block is loaded and a real device reports successful localization/tracking in its mapped physical environment.",
    "",
    "## Completion Acceptance Rules",
    "",
...
export function buildUnityEnvironmentMarkdown(report: Awaited<ReturnType<typeof buildUnityEnvironmentReport>>): string {
  return [
    ...
    `Configured executable kind: ${report.configuredInspection?.executableKind ?? "not inspected"}`,
    `Configured path resolves: ${report.configuredInspection?.resolvedPathExists ? "yes" : "no"}`,
    `Configured path supports batchmode: ${report.configuredInspection?.supportsBatchmode ? "yes" : "no"}`,
    `PATH command fallback: ${report.pathCommand}`,
    `PATH command resolved path: ${report.pathCommandInspection?.path ?? "not found"}`,
    `PATH command executable kind: ${report.pathCommandInspection?.executableKind ?? "not found"}`,
    `Active Unity path: ${report.activeUnityPath ?? "not configured"}`,
    `Active path matches project version: ${report.activeVersionMatchesProject === null ? "unknown" : report.activeVersionMatchesProject ? "yes" : "no"}`,
    `Recommended Unity path: ${report.recommendedUnityPath ?? "not found"}`,
    `Recommended executable kind: ${report.recommendedInspection?.executableKind ?? "not found"}`,
    `Recommended resolved path: ${report.recommendedInspection?.resolvedPath ?? "not found"}`,
    `Recommended detected version: ${report.recommendedInspection?.detectedVersion ?? "unknown"}`,
    ...
    "## Candidates",
    "",
    ...markdownIssueList(report.candidates.map((candidate) => `${candidate.supportsBatchmode ? "READY" : candidate.exists ? "NOT-EDITOR" : "MISSING"} ${candidate.path} (kind=${candidate.executableKind}, resolved=${candidate.resolvedPathExists ? "yes" : "no"}, executable=${candidate.executable ? "yes" : "no"}, version=${candidate.detectedVersion ?? "unknown"})`), "No Unity executable candidates were found."),
    ...(report.expectedInstall
      ? [
          "",
          "## Expected Project-Version Install",
          "",
          `Install root: ${report.expectedInstall.installRoot}`,
          `Install root is symlink: ${report.expectedInstall.installRootIsSymlink ? "yes" : "no"}`,
          `Install root resolves: ${report.expectedInstall.installRootResolvedExists ? "yes" : "no"}`,
          `Install root target: ${report.expectedInstall.installRootResolvedPath ?? "not found"}`,
          `Editor executable: ${report.expectedInstall.editorExecutable}`
        ]
      : []),
    "",
