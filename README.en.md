...
This MVP focuses on local Unity project assistance. The active sample run-through target is Image Tracking, CRS/Cloud Recognition, and Mega, including the verified PICO 4 Ultra Enterprise Mega headset path; Hello AR, Surface Tracking, and other samples stay out of scope until the user asks to continue them.

Current readiness is split into two tracks:

- Local-key MVP: ready for focused Image Tracking, Cloud Recognition, and Mega assistance when the official EasyAR Unity Plugin is installed and local license/API keys or Mega Studio materials are filled in the Unity project.
...
- generate focused runbooks and support directories for Image Tracking, Cloud Recognition, and Mega
...
- PICO 4 Ultra Enterprise: the validated baseline used EasyAR Sense Unity Plugin `4002.0.0`, EasyAR Mega `2.12.6`, EasyAR Unity XR device extension package `4000.0.0`, PICO Unity Integration SDK `3.1.0+` (validated with `3.4.0`), `PicoFrameSource`, and `LocationInputMode=Onsite` for real-device validation. The 2026-07-01 official docs now list EasyAR Sense Unity Plugin / for Mega `4003.0.0`, EasyAR Mega support package and Mega Studio `2.13.0`, and XR device extension package `4000.0.1`; for new 4003+ Mega projects, prefer the official `MegaBlockController` workflow instead of legacy Mega Studio generated node groups or BlockRoot-centered setup. If the EasyAR Simulator diagnostics caution appears in the headset, the scene is still in Simulator/non-onsite input mode and must be rebuilt after switching to Onsite. Validation is based on headset-visible VST passthrough plus Mega `Found`/block evidence.
See [docs/OFFICIAL_DOCS_2026-07-01.md](docs/OFFICIAL_DOCS_2026-07-01.md) for the current official Chinese docs refresh summary.
MCP clients can also read `easyar://official/docs-2026-07-01`.
...
See [docs/troubleshooting.md](docs/troubleshooting.md) for focused Image Tracking, Cloud Recognition, and Mega diagnostics.
...
10. `easyar_write_focused_handoff_pack` writes the safe handoff pack for Image Tracking, Cloud Recognition, Mega, or all focused samples so another AI tool can continue from `HANDOFF_PACK.md` and `ARTIFACT_INDEX.md`.
...
- `easyar_generate_focused_scope_status`: aggregate Image Tracking, Cloud Recognition, and Mega completion states into one focused-scope status.
